...
const closeModal = (confirm) => {
  isOpen.value = false;
  if (confirm) {
    router.delete(route('columns.cards.destroy', { card: props?.card?.id, column: props?.card?.column }));
  }
};
...
const isEditing = computed(() => props?.card?.id === useEditCard?.value?.currentCard);

const cardContent = computed(() => props.card?.content);

const onSubmit = () => {
  form.put(route('columns.cards.update', { column: props?.card?.column, card: props?.card?.id }), {
    onSuccess: () => {
...
<template>
  <div
      ...
      <form v-if="isEditing" @keydown.esc="onCancel" @submit.prevent="onSubmit">
        <textarea v-model="form.content" type="text"
                    ...
                    placeholder="Card content ..." ref="inputCardContentRef"
                    rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </textarea>
  ...
  <ConfirmDialog :show="isOpen" @confirm="closeModal($event)"
                 title="Remove Card"
